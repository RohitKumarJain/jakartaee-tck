...
This chapter describes how to set up the Jakarta EE 9.1 Platform TCK test suite and
configure it to work with your test environment. It is recommended that
you first set up the testing environment using the Jakarta EE 9.1 CI and then
with your Jakarta EE 9.1 server.

This chapter includes the following topics:

* link:#GEWTL[Allowed Modifications]
* link:#GEWTP[Configuring the Test Environment]
* link:#GEWTQ[Configuring a Jakarta EE 9.1 Server]
...
Jakarta EE 9.1 CI and its associated database in order to pass the TCK. A
...
1.  Identify the software pieces and assemble them into the Jakarta EE 9.1
platform to be tested for certification.
2.  Implement the porting package APIs. +
Some functionality in the Jakarta EE 9.1 platform is not completely specified
by an API. To handle this situation, the Jakarta EE 9.1 Platform TCK test suite
...
particular Jakarta EE 9.1 server environment. See
...
See link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE 9.1 CI as the
VI,"] for a list of the modifications that must be made to run CTS
against the Jakarta EE 9.1 CI.
4.  Set up the vendor's Jakarta EE 9.1 server implementation (VI). +
...
against the vendor's Jakarta EE 9.1 server.
...
5.3 Configuring a Jakarta EE 9.1 Server
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This section describes how to configure the Jakarta EE 9.1 server under test.
You can run the TCK tests against the Jakarta EE 9.1 CI or your own Jakarta
...
EE 9.1 CI servers, one of which must be a Jakarta EE 9.1 CI using, or configured to
use a database. For example, Eclipse GlassFish 6.1 is bundled and configured to use
the Apache Derby database.

For the purposes of this section, it is useful to clarify three terms as
they are used here:

* Compatible Implementation (CI): Jakarta EE 9.1 CI, for example, Eclipse GlassFish 6.1
* Vendor Implemention (VI): Jakarta EE 9.1 implementation from a vendor other
than Eclipse; typically, the goal of running the TCK is to certify a Jakarta
EE 9.1 VI; in some cases, for purposes of familiarizing yourself with TCK,
you may choose to run the Jakarta EE 9.1 CI as the VI
* Bundled Derby: Apache Derby database bundled with the Jakarta EE 9.1 CI, Eclipse GlassFish 6.1
...
There are three general scenarios for configuring Jakarta EE 9.1 servers for
Jakarta EE 9.1 Platform TCK testing (Note: in the following images, Java EE refers to Jakarta EE. 
RI should be replaced with CI for Compatible Implementation):

* Configure the Jakarta EE 9.1 CI as the server under test +
image:img/serverpath-ri.png["CI as Server Under Test"]

Use the Jakarta EE 9.1 CI as the Jakarta EE 9.1 VI; you may want to do this as a
sanity check to make sure you are comfortable with using the Jakarta EE 9.1
TCK against a known standard CI with certified sample applications
before proceeding with tests against your Jakarta EE 9.1 VI. See
link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE 9.1 CI as the VI,"]
for instructions.

* Configure your Jakarta EE 9.1 VI as Server Under Test +
image:img/serverpath-vi.png["VI as Server Under Test"]

This is the primary goal of using the Jakarta EE 9.1 Platform TCK; you will eventually
need to configure the Jakarta EE 9.1 implementation you want to certify. See
link:#GEWWA[Section 5.3.3, "Configuring Your Application Server as the
VI,"] for instructions.

* Configure two Jakarta EE 9.1 servers for the purpose of interop testing +
image:img/serverpath-interop.png["Two Servers for Interop Testing"]

Rebuildable tests require that you configure two Jakarta
EE 9.1 servers on one or two machines. One server will be your Jakarta EE 9.1
VI running a database of your choice with JDBC 4.1-compliant drivers.
The second server must be the Jakarta EE 9.1 CI using the bundled Java DB
database. 

In terms of the Jakarta EE 9.1 Platform TCK, all TCK configuration settings are made
in the `<TS_HOME>/bin/ts.jte` file. When configuring a Jakarta EE 9.1 server,
...
5.3.2 Configuring the Jakarta EE 9.1 CI as the VI
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To configure the Jakarta EE 9.1 CI as the server under test (that is, to use
the Jakarta EE 9.1 CI as the VI) follow the steps listed below. In this
scenario, the goal is simply to test the Jakarta EE 9.1 CI against the CTS
...
EE 9.1 TCK for similar instructions.
...
whatever you used when setting up the Jakarta EE 9.1 CI server.
..  Add the following JVM option for JDK11 test runs: +
+
[source,oac_no_warn]
----
...
run with JDK 8.
.  Install the Jakarta EE 9.1 CI and configure basic settings, as described
in link:install.html#GBFTP[Chapter 4, "Installation."]
.  Start the Jakarta EE 9.1 CI application server. +
Refer to the application server documentation for complete instructions.
.  Enable the Security Manager. +
If you are using the Jakarta EE 9.1 CI, execute the following command from
the command line: +
+
[source,oac_no_warn]
----
...
To use a Jakarta EE 9.1 server other than the Jakarta EE 9.1 CI, follow the steps
below.
...
using Ant configuration target similar to the target for the Jakarta EE 9.1
...
.  Start your Jakarta EE 9.1 server.

.  [[BABDADHA]] Set up required users and passwords.

..  Set up database users and passwords that are used for JDBC
connections. +
The Jakarta EE 9.1 Platform TCK requires several user names, passwords, and
...
against the Eclipse GlassFish 6.1 Jakarta EE 9.1 CI application server. It is required in this case
because the management Jakarta Enterprise Bean (MEjb) in the Jakarta EE 9.1 server is protected
...
. Configure your Jakarta EE 9.1 server to use the appropriate logical name
...
. [[BABEGCJH]] Install the client-side certificate in the `trustStore` on the Jakarta EE 9.1
server.  +
...
policy file or files on your Jakarta EE 9.1 server. +
...
Jakarta EE 9.1 server and the server that is running the Jakarta EE 9.1 CI. +
...
.. Restart your Jakarta EE 9.1 server.
...
* Set the `javaee.home.ri` property to the location where the Jakarta EE 9.1
CI is installed.

* Set the `wsgen.ant.classname` property to the Jakarta EE 9.1 application
...
----
+
* Set the `wsimport.ant.classname` property to the Jakarta EE 9.1 application
...
Before you can run any of the technology-specific Jakarta EE 9.1 Platform TCK tests,
...
When configuring the Jakarta EE 9.1 Platform TCK for the Windows environment, set the
...
used to identify the name of a logical host that processes Jakarta EE 9.1
requests. Jakarta EE 9.1 requests may be directed to a logical host using
...
Jakarta EE 9.1 profile's `AppContextId` hostname. This property is used by
the Jakarta EE 9.1 security tests as well as by the
...
Jakarta EE 9.1 CI as the VI,"] for more information about setting these
properties.
...
Jakarta RESTful Web Services tests using the Jakarta EE 9.1 Compatible Implementation and/or a
...
5.4.8.1 To Configure Your Environment to Run the Jakarta RESTful Web Services Tests Against the Jakarta EE 9.1 CI
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
5.4.8.2 To Package WAR files for Deployment on the Jakarta EE 9.1 CI
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

The Jakarta EE 9.1 Platform TCK test suite does not come with prebuilt test WAR files
for deployment on Jakarta EE 9.1 CI. The test suite includes a command to
generate the test WAR files that will be deployed on the Jakarta EE 9.1 CI.
...
To package the Jakarta RESTful Web Services WAR files for deployment on the Jakarta EE 9.1 CI,
...
vendor's Jakarta EE 9.1-compliant application server.
...
A vendor must create VI-specific Jakarta EE 9.1-compliant WAR files so the
VI-specific Servlet class will be included instead of the Jakarta EE 9.1
CI-specific Servlet class.

All resource and application class files are already compiled. The
Vendor needs to package these files. Jakarta EE 9.1 Platform TCK makes this task
easier by including template WAR files that contain all of the necessary
files except for the VI-specific servlet adaptor class. The Jakarta EE 9.1
...
descriptor also comes with the Jakarta EE 9.1 CI, Eclipse GlassFish 6.1, and has the values for the
...
The Jakarta EE 9.1 Platform TCK test suite provides a tool,
`${ts.home}/bin/xml/impl/glassfish/jersey.xml`, for the Jakarta EE 9.1 CI
...
The Jakarta Connector tests verify that a Jakarta EE 9.1 server correctly implements
the Jakarta Connector V1.7 specification. The Connector compatibility tests
ensure that your Jakarta EE 9.1 server still supports the Connector V1.0
functionality.

The `config.vi` target is run to configure the Jakarta EE 9.1 server for
...
The RAR files that are used with Jakarta EE 9.1 Platform TCK test suite differ from
those that were used in earlier test suites. Jakarta EE 9.1 Platform TCK no longer
...
See section EE.8.2.2 of the Jakarta EE 9.1 platform specification and section
20.2.0.1 in the Jakarta Connectors (formerly JCA) 1.7 specification, which explicitly state that the
resource adapter server may employ the library mechanisms in Jakarta EE 9.1.
...
Jakarta EE 9.1 Platform TCK has scenarios in which multiple standalone RAR files that
...
in the Jakarta EE 9.1 platform specification. If both RAR files (RAR1 and
...
The Jakarta EE 9.1 Platform TCK test suite treats the `whitebox.jar` dependency as an
...
See section EE.8.2.2 in the Jakarta EE 9.1 platform specification for
...
For the Eclipse GlassFish 6.1 CI, these mappings add a line to the `domain.xml` file, similar
...
Jakarta EE 9.1 CI, Eclipse GlassFish 6.1 to create this connection pool: 
+
[source,oac_no_warn]
----
...
For example, you could use the `asadmin` command line utility in the
Jakarta EE 9.1 CI to create the three connection pool elements: 
+
[source,oac_no_warn]
----
...
element, they use the same pool connection at runtime. Jakarta EE 9.1 Platform TCK
does reuse the same connection pool ID for testing the Jakarta EE 9.1 CI Eclipse GlassFish 6.1.
...
the `ts.jte` file. +
This step must be completed for Jakarta EE 9.1 and Jakarta EE 9.1 Web Profile
testing.
...
Jakarta EE 9.1 Platform TCK does not have a property that you can set to configure the
...
an XA datasource in the Jakarta EE 9.1 timer implementation. For example:
...
When testing against the Jakarta Platform, Enterprise Edition CI, Eclipse GlassFish 6.1, you must
first start the Derby DB and initialize it in addition to any other database
you may be using, as explained in link:#GEWUI[Configuring the Jakarta EE 9.1
CI as the VI].
...
The Jakarta Platform, Enterprise Edition CI, Eclipse GlassFish 6.1 provides the container-managed
...
special setup to run. This is no longer the case, since Jakarta EE 9.1 Platform TCK
...
`src/com/sun/ts/tests/ejb30/persistence` directory. The Jakarta EE 9.1 tests are now in
...
This section explains how to set up and configure the Jakarta EE 9.1 Platform TCK test
...
name of the Jakarta Messaging Resource Adapter in the Jakarta EE 9.1 CI.
...
required profile for Jakarta EE 9.1 Platform TCK. There are other optional profile
...
`<JAVAEE_HOME>` is the location of your Jakarta EE 9.1 CI installation.
...
To comply with Jakarta EE 9.1 requirements, Jakarta Authorization must be supported in both
...
When running Jakarta Authorization tests against the Jakarta EE 9.1 CI, if you need to restart
...
The Jakarta EE 9.1 Platform TCK signature tests perform verifications in two different
...
implementation needs a modification to exactly match the Jakarta EE 9.1 Platform SPEC API.
...
To configure the JavaTest harness to run the Jakarta EE 9.1 Platform TCK tests,
